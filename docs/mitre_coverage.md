...
Aman can summarize ATT&CK coverage directly from the rule catalog.

## CLI
```bash
go run ./cmd/aman system coverage -rules data/rules.json
...
MITRE ATT&CK v16 introduced **Identity Provider** and **Office Suite** platforms. Aman tags related rules as follows:
